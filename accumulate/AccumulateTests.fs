...
[<Fact>]
let ``Empty accumulation produces empty accumulation``() = accumulate (fun x -> x + 1) List.empty |> should be Empty

[<Fact>]
let ``Identity accumulation returns unmodified list``() = accumulate id [ 1; 2; 3 ] |> should equal [ 1; 2; 3 ]

[<Fact>]
let ``Accumulate squares``() = accumulate (fun x -> x * x) [ 1; 2; 3 ] |> should equal [ 1; 4; 9 ]

[<Fact>]
let ``Accumulate upcases``() =
    accumulate (fun (x: string) -> x.ToUpper()) [ "hello"; "world" ] |> should equal [ "HELLO"; "WORLD" ]
